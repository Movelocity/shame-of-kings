# Shame of Kings · 提案 v2：地图与英雄/技能并行演进（已归档）

> **已归档（2026-07-14）。** 当前生效开发计划见 [`docs/DEV.md`](../docs/DEV.md)。  
> 本文件保留作历史快照：闸口（G1/G2）+ M4 后并行叙事。排期请勿再按本文执行。  
> 原始定位：地图系统（map-design.md）与英雄扩展并行，以 `coords.ts` + 非英雄施法者类型为前置闸口。
